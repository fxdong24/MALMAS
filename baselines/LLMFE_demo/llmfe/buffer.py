...
class Island:
    ...
    def _generate_prompt(
        ...
        if random_number == 0:
            template_prefix = os.path.join(current_file_path,"prompts", "operations_head.txt")
            if os.path.exists(template_prefix):
                with open(template_prefix) as f:
                    prefix = f.read()
            template_suffix = os.path.join(current_file_path,"prompts", "operations_tail.txt")
            ...
            template_prefix = os.path.join(current_file_path,"prompts", "domain_head.txt")
            if os.path.exists(template_prefix):
                with open(template_prefix) as f:
                    prefix = f.read()
            template_suffix = os.path.join(current_file_path,"prompts", "domain_tail.txt")
            if os.path.exists(template_suffix):
                with open(template_suffix) as f:
                    suffix = f.read()
            new_prompt = new_prompt.replace('[PREFIX]', prefix).replace('[SUFFIX]', suffix)
