...
# from main_demo.model_factory import get_model
...
class LocalSandbox(Sandbox):
    ...
    def run(self, program: str, function_to_run: str, function_to_evolve: str,
        ...
        results = self._compile_and_run_function(
            program, function_to_run, function_to_evolve, dataset, self._numba_accelerate,
            result_container
        )
    ...
    def _compile_and_run_function(self, program, function_to_run, function_to_evolve,
                                  dataset, numba_accelerate, result_container):
        # print(program)
        try:
        ...
        # try :pass
            ...
            result_container.append(None)
            result_container.append(False)
        # print(f"evaluate_result_container:{result_container}")
...
class Evaluator:
    ...
    def analyse(
        ...
        if scores_per_test:
            ...
            # 保存最后一次进化的特征生成函数到本地
            if 'save_final_code' in kwargs and kwargs['save_final_code']:
                save_path = kwargs.get('final_code_path', 'final_feature_function.py')

                with open(save_path, 'w',encoding="utf-8") as f:
                    f.write(str(new_function))
