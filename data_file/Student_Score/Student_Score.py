...
class Student_Score:
    ...
    def read_data():
        ...
        1. 加载原始数据文件（默认路径为 "Student_Score.csv"）。
        ...
        csv_path = os.path.join(absolute_path, "Student_Score.csv")
        # 设置随机种子后，每次运行会得到相同结果

        # 读取原始数据
        df = pd.read_csv(csv_path)
          # 批量替换多个列
        yes_no_cols = ['Extracurricular_Activities', 'Learning_Disabilities', 'Internet_Access']
        ...
        target = target_col
        df[target_col] = df[target_col].apply(lambda x: "High Performer" if x >= 65 else "Low Performer")
