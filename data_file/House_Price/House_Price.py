...
class House_Price:
    ...
    def read_data():
        ...
        1. 加载原始数据文件（默认路径为 "House_Price.csv"）。
        ...
        csv_path = os.path.join(absolute_path, "House_Price.csv")
        ...
        description=sample_enrich_feature_description(df,description)
        ...
        if os.path.exists(enriched_description_path):
            ...
            print(f"Has loaded local file:{enriched_description_path}")
        else:
            # 文件不存在，调用生成数据
            ...
            print(f"notfound has loaded enrich_field_info_for_local_pattern() save as：{enriched_description_path}")
...
def sample_enrich_feature_description(df: pd.DataFrame, feature_info: dict) -> dict:
    """
    Enrich the feature description JSON with statistics from the DataFrame.
    Only adds numerical statistics for numeric columns.
    
    Parameters:
        df (pd.DataFrame): The input DataFrame.
        feature_info (dict): Feature metadata in dict form.
        
    Returns:
        dict: Updated feature_info with statistical properties.
    """
    for col in df.columns:
        if col in feature_info:
            series = df[col]
            feature_info[col]["missing_count"] = int(series.isna().sum())
            
            # 仅对数值列添加 min/max/mean/std
            if pd.api.types.is_numeric_dtype(series):
                feature_info[col]["min"] = float(series.min())
                feature_info[col]["max"] = float(series.max())
                feature_info[col]["mean"] = float(series.mean())
                feature_info[col]["std"] = float(series.std())
