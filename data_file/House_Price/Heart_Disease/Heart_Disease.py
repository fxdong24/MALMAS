...
def read_data_Heart_Disease():
    """
    读取数据文件并完成以下步骤：
    
    1. 加载原始数据文件（默认路径为 "Heart_Disease.csv"）。
    ...
    csv_path = os.path.join(absolute_path, "Heart_Disease.csv")

    # 读取原始数据
    df = pd.read_csv(csv_path)
    df = map_heart_disease_categories(df)
    df = df.sample(frac=1, random_state=42)  # 可以是任意整数
    ...
    print(f"Loaded local folder: {current_file_path}")
    return df_train.reset_index(drop=True), df_test.reset_index(drop=True), target_col, task_description, description, enrich_description



import pandas as pd

def map_heart_disease_categories(df: pd.DataFrame) -> pd.DataFrame:
    """Map UCI Heart Disease dataset numeric categories to human-readable labels."""
    ...
    # Apply mapping, create new columns
